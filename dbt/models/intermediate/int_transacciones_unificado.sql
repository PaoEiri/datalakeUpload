...
WITH union_transacciones AS (
    SELECT * FROM {{ ref('stg_transacciones_libre') }}
    UNION ALL
    SELECT * FROM {{ ref('stg_transacciones_segunda_mano') }}
    UNION ALL
    SELECT * FROM {{ ref('stg_transacciones_nueva') }}
    UNION ALL
    SELECT * FROM {{ ref('stg_transacciones_protegida') }}
)
...
LEFT JOIN {{ source('reference', 'dim_geografia') }} sdg
